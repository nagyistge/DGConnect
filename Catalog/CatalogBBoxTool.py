...
class CatalogBBoxTool(QgsMapToolEmitPoint):
    """
    ...
    def __init__(self, iface, ui):
        ...
        self.right = None
        # set up button
        self.ui.search_button.clicked.connect(lambda: self.search_button_clicked())
    ...
    def search_button_clicked(self):
        ...
        # can't run multiple search
            ...
            CatalogProcessForm.search_clicked(self.dialog_tool, left=self.left, right=self.right, top=self.top, bottom=self.bottom)
    ...
    def draw_new_rect(self):
        ...
        if not CatalogProcessForm.validate_bbox_fields(self.left, self.right, self.top, self.bottom, []):
            return
    ...
    def on_top(self, new_top):
        ...
        :return: None
        """
        if CatalogProcessForm.validate_is_float(new_top) and (self.top is None or float(self.top) != float(new_top)):
            self.top = new_top
            self.draw_new_rect()
    ...
    def on_bottom(self, new_bottom):
        ...
        :return: None
        """
        if CatalogProcessForm.validate_is_float(new_bottom) and (self.bottom is None or float(self.bottom) != float(new_bottom)):
    ...
    def on_left(self, new_left):
        ...
        :return: None
        """
        if CatalogProcessForm.validate_is_float(new_left) and (self.left is None or float(self.left) != float(new_left)):
            self.left = new_left
            self.draw_new_rect()
    ...
    def on_right(self, new_right):
        ...
        :return: None
        """
        if CatalogProcessForm.validate_is_float(new_right) and (self.right is None or float(self.right) != float(new_right)):
